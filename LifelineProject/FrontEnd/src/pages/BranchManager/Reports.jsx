...
const Reports = () => {
  ...
  const getProgramsReport4 = async () => {
    try {
      ...
      const response = await axios.get(`/getprogramsreport4/${branch}/`, {
        params: {
  ...
  const courseRows = activeCourses.map((course) => {
  ...
  // CHARTS AND GRAPHS
  // ENROLLED TABLE
  const labelsEnrolled = programs.map((program) => program.name);
  const dataValuesEnrolled = programs.map(
  ...
  const dataValuesGender = [genderCounts.male, genderCounts.female];
  ...
  // NATIONALITY TABLE
  const labelsNationality = ["Filipino", "Non-Filipino "]; // Example nationalities
  const dataValuesNationality = [
    nationalityCounts.filipino,
    nationalityCounts.nonFilipino,
  ]; // Replace with your actual data counts
  ...
  const dataValuesLearningMode = [
    learningModeCounts.online,
    learningModeCounts.onsite,
  ]; // Example data for learning modes
  ...
  const dataValuesSchedule = [
    scheduleCounts.weekdaysMorning,
    scheduleCounts.weekdaysAfternoon,
    scheduleCounts.weekends,
    scheduleCounts.nightClass,
  ];
  ...
  const countStudentsByGender = (students) => {
    ...
    students.forEach((student) => {
      if (student.details && student.details.sex) {
        if (student.details.sex === "M") {
          counts.male += 1;
        } else if (student.details.sex === "F") {
          counts.female += 1;
        }
      }
    });
    setGenderCounts(counts); // Update state with counts
  };

  const countStudentsByNationality = (students) => {
    const counts = { filipino: 0, nonFilipino: 0 };
    students.forEach((student) => {
      if (student.details && student.details.nationality) {
        if (student.details.nationality === "Filipino") {
          counts.filipino += 1;
        } else if (student.details.nationality === "Non-Filipino") {
          counts.nonFilipino += 1;
        }
      }
    });
    setNationalityCounts(counts); // Update state with counts
  };

  const countStudentsByLearningMode = (students) => {
    const counts = { online: 0, onsite: 0, blended: 0 };
    students.forEach((student) => {
      if (student.details && student.details.learningMode) {
        if (student.details.learningMode === "online") {
          counts.online += 1;
        } else if (student.details.learningMode === "onsite") {
          counts.onsite += 1;
        } else if (student.details.learningMode === "blended") {
          counts.blended += 1;
        }
      }
    });
    setLearningModeCounts(counts); // Update state with counts
  };
  ...
  const countStudentsBySchedule = (students) => {
    ...
    students.forEach((student) => {
      if (student.details && student.details.schedule) {
        if (student.details.schedule === "wdMF8am2pm") {
          counts.weekdaysMorning += 1;
        } else if (student.details.schedule === "wdMF2pm8pm") {
          counts.weekdaysAfternoon += 1;
        } else if (student.details.schedule === "weSS8am6pm") {
          counts.weekends += 1;
        } else if (student.details.schedule === "wd5pm9pmS8am6pm") {
          counts.nightClass += 1;
        }
      }
    });

    setScheduleCounts(counts); // Update state with counts
  };
  ...
  const studentRows = DBstudents.map((student) => ({
    id: student.user_id,
    name: student.name,
    email: student.email,
          ...
          <section className="data-section">
            ...
            <Table striped bordered hover className="data-table">
              ...
              <tbody>
                <tr>
                  <td>Filipino</td>
                  <td>{nationalityCounts.filipino}</td>
                </tr>
                <tr>
                  <td>Non-Filipino</td>
                  <td>{nationalityCounts.nonFilipino}</td>
                </tr>
              </tbody>
            </Table>
          </section>
          ...
          <section className="data-section">
            ...
            <Table striped bordered hover className="data-table">
              ...
              <tbody>
                <tr>
                  <td>Online</td>
                  <td>{learningModeCounts.online}</td>
                </tr>
                <tr>
                  <td>Onsite</td>
                  <td>{learningModeCounts.onsite}</td>
                </tr>
              </tbody>
            </Table>
          </section>
            ...
            <Table striped bordered hover className="data-table">
              ...
              <tbody>
                <tr>
                  ...
                  <td>{scheduleCounts.weekdaysMorning}</td>
                </tr>
                <tr>
                  <td>Weekdays | Monday to Friday | 2:00 PM - 8:00 PM</td>
                  <td>{scheduleCounts.weekdaysAfternoon}</td>
                </tr>
                <tr>
                  <td>Weekends | Saturday & Sunday | 8:00 AM to 6:00 PM</td>
                  <td>{scheduleCounts.weekends}</td>
                </tr>
                <tr>
                  <td>
                  ...
                  <td>{scheduleCounts.nightClass}</td>
                </tr>
              </tbody>
            </Table>
          </section>
  ...
  const logReportExport = async (reportType) => {
    ...
    try {
      const response = await axios.post("reports/", {
        generated_by: userId, // ✅ Changed from `user_id` to `generated_by`
  ...
  const generateStudentDetails = async () => {
    ...
    // Function to fetch and convert logo to Base64
  ...
  //Old copy is in the Extras.jsx under Extras Folder
  const generateSummaryList = async () => {
    ...
    if (typeof programs !== "undefined" && Array.isArray(programs)) {
      addSectionWithPageBreak(
        ...
        programs.map((p) => [p.name, p.enrolled_students]),
        "enrolled-chart"
      );
    }
